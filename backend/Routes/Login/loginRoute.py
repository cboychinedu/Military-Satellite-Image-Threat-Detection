...
def loginPage(): 
    ...
    if (userData["exists"]): 
        ...
        # Else, save the clean hex as the hexString 
        else: 
            # save the hex string as clean hex 
            cleanHex = hexString
        ...
        # Checking if the condition if the password verification is correct 
        if (condition): 
    ...
    # Else if the user data does not exist on the database 
    else: 
        # Else if the user's data is not on the database, build 
        # the response data 
        responseData = {
            "status": "error", 
            "message": "Invalid email or password!", 
